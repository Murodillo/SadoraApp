...
import { ApiFailure, query, request, requestBlob, SESSION_EXPIRED_EVENT, tokenStore } from './client'
...
describe('requestBlob', () => {
  it('sends the operator token and returns the raw bytes', async () => {
    tokenStore.write('op-token')
    const fetchMock = stubFetch(new Response(new Uint8Array([1, 2, 3]), { status: 200, headers: { 'Content-Type': 'image/png' } }))

    const blob = await requestBlob('/v1/admin/doctors/d1/documents/f1')

    expect(blob.size).toBe(3)
    const [path, init] = fetchMock.mock.calls[0] as [string, RequestInit]
    expect(path).toBe('/v1/admin/doctors/d1/documents/f1')
    expect(init.headers).toMatchObject({ Authorization: 'Bearer op-token' })
  })

  it('turns an error body into the same failure request() raises', async () => {
    stubFetch(reply(404, { error: { code: 'not_found', message: 'Hujjat topilmadi' } }))
    const failure = await failureOf(requestBlob('/v1/admin/doctors/d1/documents/missing'))
    expect(failure.code).toBe('not_found')
    expect(failure.message).toBe('Hujjat topilmadi')
