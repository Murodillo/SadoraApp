...
/*
 * The brand: the mark, the wordmark and the loader — the client app's, unchanged, so
 * the doctor app introduces itself exactly as Sadora does.
 *
 * All are drawn from the vectors in the brand deck rather than from bitmaps, so they are
 * sharp at any size, follow the theme where the deck says they should, and carry their
 * motion with them. The path data below is the deck's, unchanged; the timings are the
 * deck's too, and the comments name them where they are used.
 */
...
/** The loader's sphere — the client's AI orb, small — lit from the upper left. */
...
fun SadoraWordmark(
    ...
    /** The line under the word. The doctor app says what it is for. */
    taglineText: String = BrandTagline,
) {
    val c = Sadora.colors
